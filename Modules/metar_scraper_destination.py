...
# =============================================================================
# Function takes a dataframe with columns 'tail_number','ICAO_destination', 
# 'date', 'arr_UTC_time', and returns a folder with csv files for each tail 
# number which contains METAR data information. It also returns a concatenated 
# csv file.
  
def metar_scraper_destination(dataset):
    # import packages
    import os
    import glob
    import pandas as pd
...
# =============================================================================
    ...
    data = dataset.loc[:,['tail_number','ICAO_destination', 'date', 'arr_UTC_time']]
...
# =============================================================================
    ...
    for j in range(0,len(data)-1):
        url = "http://www.ogimet.com/display_metars2.php?lang=en&lugar=" + data.ICAO_destination.iloc[j] \
          ...
          +"&hora=" + str(pd.to_datetime(data.arr_UTC_time, format='%H:%M:%S').dt.hour.iloc[j]) + "&anof=" + str(data.date.dt.year.iloc[j]) \
          + "&mesf="+ str(data.date.dt.month.iloc[j]) + "&dayf=" + str(data.date.dt.day.iloc[j]) \
        + "&horaf=" + str(pd.to_datetime(data.arr_UTC_time, format='%H:%M:%S').dt.hour.iloc[j] + 1) + "&minf=59&send=send"
        ...
        if data.tail_number.iloc[j] == data.tail_number.iloc[j+1]:
            for i in range(len(page_soup.findAll("tr", {"bgcolor":["#F0F0D0",'white']}))):
                ...
                ICAO_destination += [data.ICAO_destination.iloc[j]]
        ...
        if j == len(data)-2 and data.tail_number.iloc[j] == data.tail_number.iloc[j+1]:
            url = "http://www.ogimet.com/display_metars2.php?lang=en&lugar=" + data.ICAO_destination.iloc[-1] \
              ...
              +"&hora=" + str(pd.to_datetime(data.arr_UTC_time, format='%H:%M:%S').dt.hour.iloc[-1]) + "&anof=" + str(data.date.dt.year.iloc[-1]) \
              + "&mesf="+ str(data.date.dt.month.iloc[-1]) + "&dayf=" + str(data.date.dt.day.iloc[-1]) \
              + "&horaf=" + str(pd.to_datetime(data.arr_UTC_time, format='%H:%M:%S').dt.hour.iloc[-1] + 1) + "&minf=59&send=send"
            ...
            for i in range(len(page_soup.findAll("tr", {"bgcolor":["#F0F0D0",'white']}))):
                ...
                ICAO_destination += [data.ICAO_destination.iloc[-1]]
                date += [page_soup.findAll("tr", {"bgcolor":["#F0F0D0",'white']})[i].findAll('td')[1].text.replace('->','')]
                METAR += [page_soup.findAll("tr", {"bgcolor":["#F0F0D0",'white']})[i].findAll('td')[2].text.replace('\n          ', '')]
            
            # export as csv
            pd.DataFrame(list(zip(tail_number, ICAO_destination, date, METAR)), columns= ['tail_number', 'ICAO_destination', 'date', 'METAR'])\
            .to_csv(os.getcwd() + '/destination/' + data.tail_number.iloc[-1] + '.csv', index=False) 
        ...
        if data.tail_number.iloc[j] != data.tail_number.iloc[j+1] and j != len(data)-2:
            ...
            # export as csv             
            df = pd.DataFrame(list(zip(tail_number, ICAO_destination, date, METAR)), columns= ['tail_number', 'ICAO_destination', 'date', 'METAR'])
...
#                            concat all files
# =============================================================================
    all_filenames = [i for i in glob.glob(os.getcwd() + '/destination/' + '*.{}'.format('csv'))]
    
    #combine all files in the list
    combined_csv = pd.concat([pd.read_csv(f) for f in all_filenames ])
    
    #export to csv
    combined_csv.to_csv(os.getcwd() + '/destination/' + "METAR_destination.csv", index=False, encoding='utf-8-sig')
